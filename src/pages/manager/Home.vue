<template>
  <div class="home">
    <img style="width:100%;margin:0 auto" src="../../assets/images/timg56.jpg" alt />
    <!-- <van-loading v-show="visible" type="spinner" color="#1989fa" size="30px" style="padding:240px 0;text-align:center;">加载中...</van-loading> -->
    <!-- 待接单 -->
    <van-panel
      :title="item.customer.realname"
      :status="item.status"
      v-for="item in orderStatusFilter('待接单')"
      :key="item.b"
    >
    ...
    >
      <div style="font-size:14px;color:#5a5e5e;">
        ...
        <div
          slot="footer"
          v-show="item.status == '待服务'"
          style="text-align:right;margin-right:16px;"
        >
          <van-button size="small" type="warning" plain @click="confirmHandler(item.id)">派送完成</van-button>&nbsp;
        </div>
      </div>
    </van-panel>
    <!-- {{info}} -->
...
export default {
  ...
  methods: {
    ...
    rejectHandler(orderId) {
      ...
      this.rejectOrder(orderId).then(response => {
    ...
    acceptHandler(orderId) {
      ...
      this.acceptOrder(orderId).then(response => {
        this.$toast(response.statusText)
      })
    },
    //完成订单
    confirmHandler(orderId) {
      // console.log("接受的订单",orderId)
      this.confirmOrder(orderId).then(response => {
        this.$toast(response.statusText)
      })
    }
  }
